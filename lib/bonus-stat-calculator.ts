...
export interface BonusStatCalcInput {
    ...
    isBossDrop: boolean;
    aimStat: number; // e.g. 100급 (방어구용)
    goalMode?: 'SCORE' | 'CUSTOM_STATS'; // SCORE(급수) or CUSTOM_STATS(직접입력)
    targetStats?: {
        STR?: number;
        DEX?: number;
        INT?: number;
    ...
    minBossDamage?: number; // 무기 보스 데미지 % (예: 10, 12, 14)
    minDamage?: number;     // 무기 일반 데미지 % (예: 5, 6, 7)
    minAllStat?: number;    // 무기 올스탯 % (예: 5, 6, 7)
...
function getPercentileAttempts(prob: number, targetPercent: number): number | null {
...
export function calculateBonusStat(input: BonusStatCalcInput): BonusStatCalcOutput {
    const { equipLevel, equipType, isBossDrop, goalMode = 'SCORE', aimStat, targetStats, weaponGrade, minBossDamage, minDamage, minAllStat, statEfficiency } = input;
    const optionTable = generateBonusOptionTable(equipLevel, isBossDrop, equipType);
    const hasTargetStats = goalMode === 'CUSTOM_STATS' && targetStats && Object.values(targetStats).some(v => (v || 0) > 0);
    ...
    for (const key of ALL_OPTION_KEYS) {
        ...
        adjustedTable[key] = optionTable[key].map(val => {
            if (equipType === 'WEAPON' && key === 'ATTACK') return 0; // Handled separately by weaponGrade
    ...
    for (const count of selectCounts) {
        ...
        for (const optCombo of optionCombos) {
            ...
            const bossIdx = optCombo.indexOf("BOSS_DAMAGE_OR_SPEED");
            const damIdx = optCombo.indexOf("DAMAGE_OR_JUMP");
            const allIdx = optCombo.indexOf("ALL %");
            ...
            if (!hasTargetStats && equipType !== 'WEAPON' && minPossible >= aimStat) {
                methodKeys.forEach(m => comboProbSum[m] += 1);
                continue;
            }
            if (!hasTargetStats && equipType !== 'WEAPON' && maxPossible < aimStat) {
                continue;
            }
            ...
            for (const tiers of tierCombos) {
                if (equipType === 'WEAPON') {
                    ...
                    if (minBossDamage && minBossDamage > 0) {
                        const bossVal = bossIdx === -1 ? 0 : optionTable.BOSS_DAMAGE_OR_SPEED[tiers[bossIdx]];
                        if (bossVal < minBossDamage) continue;
                    }
                    if (minDamage && minDamage > 0) {
                        const damVal = damIdx === -1 ? 0 : optionTable.DAMAGE_OR_JUMP[tiers[damIdx]];
                        if (damVal < minDamage) continue;
                    }
                    if (minAllStat && minAllStat > 0) {
                        const allVal = allIdx === -1 ? 0 : optionTable['ALL %'][tiers[allIdx]];
                        if (allVal < minAllStat) continue;
                    }
                    if (aimStat > 0) {
                        let sumScore = 0;
                        ...
                        if (sumScore < aimStat) continue;
                    }
                } else if (hasTargetStats) {
                    // Custom stats evaluation
                    let str = 0, dex = 0, intVal = 0, luk = 0, allStat = 0, att = 0, magicAtt = 0, hp = 0;
                    for (let i = 0; i < count; i++) {
                        const optKey = optCombo[i];
                        const val = optionTable[optKey][tiers[i]];
                        if (optKey === 'STR') str += val;
                        else if (optKey === 'DEX') dex += val;
                        else if (optKey === 'INT') intVal += val;
                        else if (optKey === 'LUK') luk += val;
                        else if (optKey === 'STR+DEX') { str += val; dex += val; }
                        else if (optKey === 'STR+INT') { str += val; intVal += val; }
                        else if (optKey === 'STR+LUK') { str += val; luk += val; }
                        else if (optKey === 'DEX+INT') { dex += val; intVal += val; }
                        else if (optKey === 'DEX+LUK') { dex += val; luk += val; }
                        else if (optKey === 'INT+LUK') { intVal += val; luk += val; }
                        else if (optKey === 'ALL %') allStat += val;
                        else if (optKey === 'ATTACK') att += val;
                        else if (optKey === 'MAGIC_ATTACK') magicAtt += val;
                        else if (optKey === 'HP') hp += val;
                    }

                    if (targetStats?.STR && str < targetStats.STR) continue;
                    if (targetStats?.DEX && dex < targetStats.DEX) continue;
                    if (targetStats?.INT && intVal < targetStats.INT) continue;
                    if (targetStats?.LUK && luk < targetStats.LUK) continue;
                    if (targetStats?.['ALL %'] && allStat < targetStats['ALL %']) continue;
                    if (targetStats?.ATTACK && att < targetStats.ATTACK) continue;
                    if (targetStats?.MAGIC_ATTACK && magicAtt < targetStats.MAGIC_ATTACK) continue;
                    if (targetStats?.HP && hp < targetStats.HP) continue;
                } else {
                    // Score evaluation
                    let sumScore = 0;
                    for (let i = 0; i < count; i++) {
    ...
    for (const m of methodKeys) {
        ...
        const isAchievable = p > 0;
        const attempts = isAchievable ? Math.round(1 / p) : null;
        const mesoCost = def.costPerTry > 0 && attempts != null ? attempts * def.costPerTry : undefined;
