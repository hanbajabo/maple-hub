...
export function getLatestItemPrices(): Record<string, number> {
    ...
    const ALIAS_TO_CANONICAL: Record<string, string[]> = {
        ...
        '거공':     ['거대한 공포'],
        '몽벨':     ['몽환의 벨트'],
        '미트라':   ['미트라의 분노', '미트라의 분노 : 전사', '미트라의 분노 : 궁수', '미트라의 분노 : 마법사', '미트라의 분노 : 도적', '미트라의 분노 : 해적'],
        '황홀한 악몽': ['황홀한 악몽'],
        '죽음의 맹세': ['죽음의 맹세'],
        '근원의 속삭임': ['근원의 속삭임'],
        '불멸의 유산': ['불멸의 유산'],
...
export function getJobCategory(jobName?: string): 'warrior' | 'mage' | 'archer' | 'thief' | 'pirate' {
    ...
    const WARRIORS = ['히어로', '팔라딘', '다크나이트', '소울마스터', '미하일', '블래스터', '데몬슬레이어', '데몬어벤져', '아란', '카이저', '아델', '제로', '렌'];
    const MAGES = ['아크메이지(불,독)', '아크메이지(썬,콜)', '비숍', '플레임위자드', '배틀메이지', '에반', '루미너스', '일리움', '라라', '키네시스', '레테', '린'];
