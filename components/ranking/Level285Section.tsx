...
export default function Level285Section() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await fetch('/api/level285');
    ...
    if (loading) {
        return (
            <div className="bg-blue-900/20 backdrop-blur-sm rounded-xl p-8 border border-blue-500/20 mt-12">
    ...
    return (
        <div className="space-y-6 sm:space-y-8 mt-6 sm:mt-12 pt-6 sm:pt-12 border-t-2 border-blue-500/30">
            {/* 헤더 */}
            <div className="text-center">
                <h2 className="text-2xl sm:text-4xl font-bold text-white mb-4 flex items-center justify-center gap-2 sm:gap-3">
                    <span>⚡</span> Lv.285+ 초고레벨 현황
                </h2>
                <p className="text-blue-200 text-sm sm:text-lg">
                    메이플스토리 최상위 레벨 유저 전체 통계
                    <span className="ml-2 text-xs sm:text-sm text-blue-400 block sm:inline mt-1 sm:mt-0">(매일 오전 1시 업데이트)</span>
                </p>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8 gap-2 sm:gap-3">
                {sortedLevels.map(level => (
                    <div key={level} className={`
                        rounded-xl p-2 sm:p-3 border text-center
                        ${level === 300
                    ...
                    `}>
                        <p className={`text-xs sm:text-sm font-bold mb-1 ${level === 300 ? 'text-yellow-400' :
                                level >= 295 ? 'text-purple-300' :
                                    'text-blue-300'
                            }`}>
                            Lv.{level}
                        </p>
                        <p className="text-lg sm:text-xl font-bold text-white">
                            {levelStats[level]}<span className="text-xs text-gray-400 ml-1">명</span>
                        </p>
                    </div>
                ))}
                <div className="bg-emerald-900/20 border border-emerald-500/30 rounded-xl p-2 sm:p-3 text-center">
                    <p className="text-emerald-400 text-xs sm:text-sm font-bold mb-1">총 인원</p>
                    <p className="text-lg sm:text-xl font-bold text-white">
                        {allPlayers.length}<span className="text-xs text-gray-400 ml-1">명</span>
                    </p>
                </div>
            </div>

            {/* 랭킹 리스트 */}
            <div className="bg-blue-900/20 backdrop-blur-sm rounded-xl overflow-hidden border border-blue-500/20">
                <div className="p-4 sm:p-6 border-b border-blue-500/20 flex flex-col sm:flex-row justify-between items-center gap-4 sm:gap-0">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
                        <div className="relative w-full sm:w-64">
                            <input
                                ...
                                className="w-full bg-slate-900/50 border border-blue-500/30 rounded-lg py-2 px-4 pl-10 text-sm text-blue-100 placeholder-blue-400/50 focus:outline-none focus:border-blue-400 transition-colors"
                            />
                        ...
                        <span className="text-xs sm:text-sm text-blue-300 whitespace-nowrap">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-blue-100 min-w-[500px] sm:min-w-0">
                        <thead>
                            <tr className="bg-blue-600/30 text-blue-100">
                        ...
                        <tbody>
                            {filteredPlayers.length > 0 ? (
                                filteredPlayers.slice(0, displayCount).map((player, index) => (
                                    <tr key={index} className="border-b border-blue-700/30 hover:bg-blue-500/10 transition-colors">
                                        <td className="px-3 py-2 sm:px-6 sm:py-3 font-bold text-blue-400 text-xs sm:text-base whitespace-nowrap">
                                            {player.ranking}
                                        </td>
                                        ...
                                        <td className="px-3 py-2 sm:px-6 sm:py-3 text-center text-blue-200 text-xs sm:text-base whitespace-nowrap">
                                            {player.world_name}
                                        </td>
                                        <td className="px-3 py-2 sm:px-6 sm:py-3 text-center text-blue-200 text-xs sm:text-base whitespace-nowrap">
                                        ...
                                        <td className="px-3 py-2 sm:px-6 sm:py-3 text-center">
                                            <span className={`font-bold px-2 py-1 rounded text-xs sm:text-base ${player.character_level === 300 ? 'bg-yellow-500/20 text-yellow-400' :
                                                    player.character_level >= 295 ? 'bg-purple-500/20 text-purple-400' :
                                                        player.character_level >= 290 ? 'bg-red-500/20 text-red-400' :
                                                            'text-cyan-400'
                                                }`}>
                                ...
                                <tr>
                                    <td colSpan={5} className="py-12 text-center text-blue-300">
                                        검색 결과가 없습니다.
                                    </td>
                                </tr>
                            )}
                ...
                {displayCount < filteredPlayers.length && (
                    <div className="p-4 text-center border-t border-blue-500/20">
                        <button
                            ...
                            className="px-6 py-2 bg-blue-600/50 hover:bg-blue-600 text-white rounded-lg transition-all text-sm sm:text-base font-semibold"
                        >
