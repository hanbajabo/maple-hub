...
async function run() {
    ...
    const parsedItems = items.map(item => {
        return {
            ...
            baseLevel: item.item_base?.req_level || 250
        };
    });

    fs.writeFileSync('scratch/adel_items_full.json', JSON.stringify(parsedItems, null, 2), 'utf8');
    console.log('Saved parsed items to scratch/adel_items_full.json');

    console.log('\n--- Key Items Summary for 아델 (전 서버 2위 랭커) ---');
    parsedItems.forEach(i => {
        if (i.starforce > 0) {
            console.log(`[${i.slot}] ${i.name} -> ${i.starforce}성 (${i.potentialGrade || '노잠재'} / 에디:${i.additionalGrade || '없음'}) [${i.potential1}, ${i.potential2}, ${i.potential3}]`);
        }
    });
}
