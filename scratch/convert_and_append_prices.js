...
const ethernelMap = {
    ...
    '에테르넬 메이지살렛': { slot: '모자', job: '마' },
    '에테르넬 아처후드': { slot: '모자', job: '궁' },
    '에테르넬 시프버거넷': { slot: '모자', job: '도' },
    '에테르넬 파이렛레더햇': { slot: '모자', job: '해' },
    // 상의
    '에테르넬 나이트아머': { slot: '상의', job: '전' },
    '에테르넬 메이지코트': { slot: '상의', job: '마' },
    '에테르넬 아처슈트': { slot: '상의', job: '궁' },
    '에테르넬 시프슈트': { slot: '상의', job: '도' },
    ...
    // 신발
    '에테르넬 나이트부츠': { slot: '신발', job: '전' },
    '에테르넬 메이지부츠': { slot: '신발', job: '마' },
    '에테르넬 아처부츠': { slot: '신발', job: '궁' },
    '에테르넬 시프부츠': { slot: '신발', job: '도' },
    '에테르넬 파이렛부츠': { slot: '신발', job: '해' },
    // 장갑
    '에테르넬 나이트건틀렛': { slot: '장갑', job: '전' },
    '에테르넬 메이지글러브': { slot: '장갑', job: '마' },
    '에테르넬 아처글러브': { slot: '장갑', job: '궁' },
    '에테르넬 시프글러브': { slot: '장갑', job: '도' },
    '에테르넬 파이렛글러브': { slot: '장갑', job: '해' },
    // 망토
    '에테르넬 나이트클록': { slot: '망토', job: '전' },
    '에테르넬 메이지클록': { slot: '망토', job: '마' },
    '에테르넬 아처클록': { slot: '망토', job: '궁' },
    '에테르넬 시프클록': { slot: '망토', job: '도' },
    '에테르넬 파이렛클록': { slot: '망토', job: '해' },
};

// 미트라 & 마도서
// 미트라의 분노 : 전사 / 궁수 / 마법사 / 도적 / 해적 -> 미트라 평균값
// 저주받은 적/녹/청/황의 마도서 -> 마도서 평균값
...
function toEok(price) {
    ...
    if (eok >= 100) {
        ...
        return Math.round(eok * 100) / 100;
    } else if (eok >= 1) {
        return Math.round(eok * 100) / 100;
    } else {
        return Math.round(eok * 1000) / 1000;
    }
}

// Group all data by date
const dateMap = {}; // { '2026-04-16': { challenger: {}, main: {}, ethernel: {}, radiant: {} } }
...
Object.keys(dateMap).forEach(date => {
    ['challenger', 'main'].forEach(sKey => {
        ...
        if (sObj._mitraList && sObj._mitraList.length > 0) {
            ...
            sObj['미트라'] = Math.round(avg * 100) / 100;
        ...
        if (sObj._madosoList && sObj._madosoList.length > 0) {
            ...
            sObj['마도서'] = Math.round(avg * 100) / 100;
            delete sObj._madosoList;
        }
    });
});

console.log(`Prepared ${Object.keys(dateMap).length} dates.`);
const sortedDates = Object.keys(dateMap).sort();
console.log("Oldest date:", sortedDates[0]);
console.log("Newest date:", sortedDates[sortedDates.length - 1]);

// Let's generate the markdown for all dates and check a sample
...
console.log("Saved generated markdown to scratch/generated_prices_apr_to_aug.md!");
