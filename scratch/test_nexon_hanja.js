...
async function run() {
    ...
    if (!ocidRes.ocid) {
        console.error('Character not found on Nexon API!');
        return;
    }
    ...
    // Save full JSON
    fs.writeFileSync('scratch/hanja_nexon_full.json', JSON.stringify({ basic: basicRes, stat: statRes, hexa: hexaRes }, null, 2), 'utf8');
}
