    ...
    try {
        ...
        const data = await page.evaluate(() => {
            ...
            if (seedTitle) {
                ...
                // Go up 3 levels to find the main container
                ...
                if (greatGrandParent) {
                    const text = greatGrandParent.innerText;
                    const lines = text.split('\n').map(l => l.trim()).filter(l => l.length > 0);

                    // Logic: Find line with "링", then look ahead for Level and %
                    for (let i = 0; i < lines.length; i++) {
                        const line = lines[i];
                        if (line.includes('링') && !line.includes('시드링')) { // Exclude title
                            // Try to construct the full string
                            // Case 1: "Restraint Ring" -> "4 Level" -> "55%"
                            // Case 2: "Restraint Ring 4 Level" -> "55%"
                            ...
                            if (stats) {
                                result.seedRings.push(`${ringName}: ${stats}`);
                            }
                        }
                    }
                }
...
async function main() {
    const results = [];
    console.log(`Starting SEED RING crawl for ${TARGET_JOBS.length} jobs...`);
    ...
    let csvContent = "직업,시드링 통계 (4억+)\n";
    results.forEach(row => {
        const stats = row.data && row.data.seedRings ? row.data.seedRings.join(" / ") : "N/A";
        csvContent += `${row.job},"${stats}"\n`;
    });
    fs.writeFileSync('maple_seed_ring_stats.csv', csvContent, 'utf8');
    console.log("Crawl complete. Data saved to maple_seed_ring_stats.csv");
}
