...
// Full list of MapleStory jobs (excluding some special/beginner classes if needed)
const TARGET_JOBS = [
    '히어로', '팔라딘', '다크나이트',
...
async function main() {
    const results = [];
    console.log(`Starting crawl for ${TARGET_JOBS.length} jobs...`);
    ...
    fs.writeFileSync('maple_hat_stats.json', JSON.stringify(results, null, 2));
    console.log("Crawl complete. Data saved to maple_hat_stats.json");

    // Convert to CSV for Excel
    let csvContent = "직업,쿨타임 감소 통계\n";
    results.forEach(row => {
        const stats = row.data && row.data.hatPotentials ? row.data.hatPotentials.join(" / ") : "N/A";
        csvContent += `${row.job},"${stats}"\n`;
    });
    fs.writeFileSync('maple_hat_stats.csv', csvContent, 'utf8'); // Add BOM if needed for Excel: '\ufeff' + csvContent
    console.log("CSV saved to maple_hat_stats.csv");
}
