          ...
          <div className="mt-6 bg-amber-950/40 border border-amber-600/40 rounded-xl p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                ...
                { name: '칠흑 장신구', desc: '고레벨 핵심 장신구. 장기 사용 전제라면 레전드리 투자 가치 있음', tag: '조건부', tagColor: 'bg-yellow-500/20 text-yellow-300 border-yellow-500/30', warn: '⚠️ 노작 칠흑을 등급업해서 쓸 분들은 장교불(장비 착용시 교환 불가) 상태에서 돌리세요! 카블큐는 장착 시 가횟이 깎이고, 타 직업 스탯이 뜨면 귀속 후 팔 수 없게 됩니다.' },
                { name: '에테르넬 방어구', desc: '현 최종 방어구. 장기 사용을 전제로 꾸준히 직접 천천히 스타포스와 잠재능력을 직작해서 쓸 유저들에게 추천', tag: '조건부', tagColor: 'bg-yellow-500/20 text-yellow-300 border-yellow-500/30' },
                { name: '18성 이상 악세서리', desc: '별이 높아 교체가 어려운 악세라면 레전드리 도전 고려 가능', tag: '조건부', tagColor: 'bg-yellow-500/20 text-yellow-300 border-yellow-500/30' },
                { name: '페어리 하트', desc: '레전드리 잠재 확정 주문서 보유 시 우선 바를 것.', tag: '확정만', tagColor: 'bg-blue-500/20 text-blue-300 border-blue-500/30' },
              ].map(({ name, desc, tag, tagColor, warn }) => (
                <div key={name} className={`bg-slate-900/60 border rounded-lg p-3.5 ${warn ? 'border-red-600/50' : 'border-slate-600'}`}>
                  ...
                  {warn && (
                    <p className="mt-2 text-xs text-red-300 bg-red-950/50 border border-red-700/40 rounded-lg px-3 py-2 leading-relaxed">{warn}</p>
                  )}
                </div>
              ))}
            </div>
