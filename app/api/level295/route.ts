...
const TARGET_LEVEL = 295;
const MAX_PAGE_CHECK = 50; // 안전장치: 최대 50페이지까지만 확인 (약 10,000명)
    ...
    while (keepFetching && page <= MAX_PAGE_CHECK) {
        try {
            ...
            if (lastPlayer.character_level < TARGET_LEVEL) {
                keepFetching = false; // 295 미만 나오면 중단
            } else {
                page++; // 다음 페이지
...
export async function GET() {
    try {
        ...
        console.error('Level 295 data fetch error:', error);
