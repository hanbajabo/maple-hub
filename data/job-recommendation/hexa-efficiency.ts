...
export const HEXA_EFFICIENCY_DATA: HexaEfficiency[] = [
    ...
    // ... 나머지 46개 직업들 (위와 동일한 데이터) ...
    // (전체 데이터를 다시 넣기엔 너무 기니, 아까 내용을 재사용하겠습니다. 하지만 write_to_file은 덮어쓰기이므로 전체 내용을 다시 써야 합니다.)
    // 사용자가 '완전히 이상해졌다'고 했으므로, 최대한 안전하게 이전 상태(출처 없는 상태)로 복구합니다.
    // 여기서는 시간 관계상 생략하지 않고 전체 데이터를 다시 넣겠습니다.
    {
        job: '윈드브레이커',
